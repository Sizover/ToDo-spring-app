...
//        tools.logonTool()
        ...
        var subMenu = MyMenu.Dictionaries.Municipalities
        for (i in 1..5){
            when(i){
                1 -> {subMenu = MyMenu.Dictionaries.Municipalities}
                2 -> {subMenu = MyMenu.Dictionaries.Officials}
                3 -> {subMenu = MyMenu.Dictionaries.Hotlines}
                4 -> {subMenu = MyMenu.Dictionaries.VideoCameras}
                5 -> {subMenu = MyMenu.Dictionaries.Labels}
            }
    ...
    fun `PMI 0030 Проверка наличия Организаций с метками ОМПЛ, ПОО и СЗО в справочнике «Организации»`() {
        ...
        menuNavigation(MyMenu.Dictionaries.Companies, waitTime)
    ...
    fun `PMI 0090 Проверка загрузки таблицы «Архив происшествий»`() {
        ...
        menuNavigation(MyMenu.Incidents.IncidentsArchive, waitTime)
...
//    @org.testng.annotations.Test (retryAnalyzer = Retry::class, groups = ["ПМИ", "ALL"])
    ...
    fun `PMI 0120 Проверка наличия опросника абонента (заявителя)`() {
        ...
        element(byXpath("//*[text()='Опросник']/ancestor::button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        element(byXpath("//*[text()='Опросник']/ancestor::button//*[@d='M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z']"))
            .should(exist, ofSeconds(waitTime))
        element(byXpath("//*[text()='Опросник']/ancestor::button//*[@d='M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z']"))
            .shouldNot(exist, ofSeconds(waitTime))
    ...
    fun `PMI 0130 Регистрация вызова (формирование карточки происшествия) с проверкой выполнения плана реагирования`() {
        ...
        element(byXpath("//*[text()='Реагирование']/ancestor::button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
            .click()
...
//                element(byCssSelector("div#simple-tabpanel-iplan > div > div > div > div:nth-child($p) > div > div#panel1a-header div.MuiBox-root > div:nth-child($c) > span[title^='Статус: Не выполнен'] rect[fill='#16BA13']"))
    ...
    fun `PMI 0140 Проверка фильтрации карточек происшествия`() {
        ...
        setFilterByEnum(FilterEnum.Типы_происшествий, "К Консультации;Л Ложные", waitTime)
...
//        println("intS $intS")
        ...
        setFilterByEnum(FilterEnum.Уровни, "Угроза ЧС;ЧС", waitTime)
        //Дожидаемся применения фильтра
        Thread.sleep(2000)
        ...
        setFilterByEnum(FilterEnum.Источники, "Видеоаналитика;СМС", waitTime)
...
//        for (i in 1..10){
        MyMenu.Dictionaries.values().forEach {     it ->
            ...
            element(byCssSelector("main table>tbody"))
                .should(exist, ofSeconds(waitTime))
                .shouldBe(visible, ofSeconds(waitTime))
            if (it != MyMenu.Dictionaries.IncidentTypes){
                element(byXpath("//table/tbody/tr"))
                    .should(exist, ofSeconds(waitTime))
                    .shouldBe(visible, ofSeconds(waitTime))
                ...
                element(byXpath("//table/tbody/tr[1]/td[1]"))
                    ...
                    .shouldBe(visible, ofSeconds(waitTime))
                    .click()
            } else if (it == MyMenu.Dictionaries.IncidentTypes){
                element(byXpath("//table/tbody/tr"))
                    .should(exist, ofSeconds(waitTime))
                    .shouldBe(visible, ofSeconds(waitTime))
                    .click()
    ...
    open fun Справочники(): Any {
        return arrayOf<Any>(
            MyMenu.Dictionaries.values()
//            arrayOf(MyMenu.Dictionaries.values()),
//            arrayOf(MyMenu.Dictionaries.CumulativePlans),
//            arrayOf(MyMenu.Dictionaries.VideoCameras),
//            arrayOf(MyMenu.Dictionaries.Sensors),
//            arrayOf(MyMenu.Dictionaries.Hotlines),
//            arrayOf(MyMenu.Dictionaries.Officials),
//            arrayOf(MyMenu.Dictionaries.Labels),
//            arrayOf(MyMenu.Dictionaries.Municipalities),
//            arrayOf(MyMenu.Dictionaries.Companies),
//            arrayOf(MyMenu.Dictionaries.HotlineAssets),
//            arrayOf(MyMenu.Dictionaries.IncidentTypes)
        )
    }
                ...
                //проверяем существует ли заголовок столбца, и если существует, то:
                    ...
                    for (hint in 0 until searchHintList!!.size){
                        ...
                        if (columnName.contains(searchHintList[hint])
                            ...
                            if (subMenu != MyMenu.Dictionaries.IncidentTypes) {
                            ...
                            } else if (subMenu == MyMenu.Dictionaries.IncidentTypes) { //Отдельно обрабатываем справочник типов происшествий
...
//        }
    ...
    fun `PMI 0250 Проверка присвоения и удаления меток в карточке организации`(){
        ...
        //ждем Редактировать
        element(byXpath("//*[text()='Изменить']/ancestor::*[@title='Редактировать']//button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        ...
        var amountLabels = elements(byXpath("//div[@id='labels']//span[@title]//span[text()]")).size
        val oldLabelsList = mutableListOf<String>()
        val newLabelsList = mutableListOf<String>()
        if (amountLabels > 0){
            for (i in 1..amountLabels){
                oldLabelsList.add(element(byXpath("//div[@id='labels']//span[@title][$i]//span[text()]")).ownText)
            }
        }
        //жмем Редактировать
        element(byXpath("//*[text()='Изменить']/ancestor::*[@title='Редактировать']//button"))
        ...
        //пересчитываем метки
        amountLabels = elements(byXpath("//label[text()='Метки']/..//span[@title]//span[text()]")).size
        //вносим каждую в список
        for (i in 1..amountLabels){
            newLabelsList.add(element(byXpath("//label[text()='Метки']/..//span[@title][$i]//span[text()]")).ownText)
        }
        element(byXpath("//span[text()='Сохранить']/parent::button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        ...
        newLabelsList.forEach {label ->
            element(byXpath("//div[@id='labels']//span[@title]//span[text()='$label']"))
                .should(exist, ofSeconds(waitTime))
        }
        logoffTool()
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        newLabelList.forEach {
            element(byXpath("//label[text()='Метки']/..//span[text()='$it']/ancestor::span[@title]//*[name()='svg']"))
                .should(exist, ofSeconds(waitTime))
                .shouldBe(visible, ofSeconds(waitTime))
                .click()
        }
        ...
        newLabelList.forEach {
            element(byXpath("//div[@id='labels']//span[@title]//span[text()='$it']"))
                .shouldNot(exist, ofSeconds(waitTime))
                .shouldNotBe(visible, ofSeconds(waitTime))
        }
        oldLabelsList.forEach {
            element(byXpath("//div[@id='labels']//span[@title]//span[text()='$it']"))
                .should(exist, ofSeconds(waitTime))
                .shouldBe(visible, ofSeconds(waitTime))
        }
    ...
    fun `PMI 0260 Проверка использования ассоциативных связей-ссылок между объектами справочников`(){
        ...
        //кликаем по иконке справочников
        //переходим в нужный справочник
        menuNavigation(MyMenu.Dictionaries.Hotlines, waitTime)
        //ждем загрузки таблицы
        element(byCssSelector("main table>tbody"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        //Собираем все ФИО, что бы не проверять организацию у которой руководитель не указан
        tableCheckbox("Руководитель", true, waitTime)
        ...
        element(byXpath("//table/tbody/tr[$rndOrganization]/td[1]")).click()
        element(byCssSelector("form"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        //запоминаем ФИО руководителя
        element(byXpath("//main//div[@id='contacts']//*[text()='Руководитель']/..//span[@title='Перейти']//button/ancestor::*[text()]"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        val officialFIO = element(byXpath("//main//div[@id='contacts']//*[text()='Руководитель']/..//span[@title='Перейти']//button/ancestor::*[text()]"))
            .ownText
        //кликаем на стрелку перехода к руководителю
        element(byXpath("//main//div[@id='contacts']//*[text()='Руководитель']/..//span[@title='Перейти']//button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
            .click()
        element(byXpath("//main//h1[text()='$officialFIO']"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
        element(byXpath("//span[@title='Редактировать']//*[text()='Изменить']/ancestor::button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
            .click()
        ...
        element(byXpath("//form//*[text()='Отмена']/ancestor::button"))
            .should(exist, ofSeconds(waitTime))
            .shouldBe(visible, ofSeconds(waitTime))
            .click()
        element(byXpath("//main//h1[text()='$officialFIO']"))
...
//    @org.testng.annotations.Test (retryAnalyzer = Retry::class, groups = ["ПМИ", "ALL"])
    ...
    menuNavigation(MyMenu.Dictionaries.Officials, waitTime)
        tableCheckbox("", true, waitTime)
        //воспользуемся поиском
        var menuColumnNubber = tableNumberOfColumn(" ", waitTime)
        ...
        val userColumn = tableNumberOfColumn("Пользователь", waitTime)
...
//        tools.checkbox("Организация", true, waitTime)
    ...
    element(byCssSelector("main table>tbody"))
        .should(exist, ofSeconds(waitTime))
        .shouldBe(visible, ofSeconds(waitTime))
    menuColumnNubber = tableNumberOfColumn(" ", waitTime)
...
//        element(byXpath("//label[text()='Фамилия']/following-sibling::div/input")).sendKeys(dateTime)
    element(byXpath(addAtributeSelector.format("Фамилия"))).sendKeys(dateTime.toString())
    ...
    element(byCssSelector("input[placeholder^='ФИО']"))
        ...
        .shouldBe(visible, ofSeconds(waitTime))
        .click()
    element(byCssSelector("input[placeholder^='ФИО']")).sendKeys(dateTime.toString(), Keys.ENTER)
    //открываем три точки
